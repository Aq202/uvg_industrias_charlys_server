 ...
 begin
 	SELECT CAST(SUBSTRING(id_client_organization FROM 4) AS INTEGER) INTO prev_id_number FROM "client_organization" ORDER BY id_client_organization DESC LIMIT 1;
	IF (prev_id_number IS NULL) THEN
		prev_id_number = 0;
		END IF;
	NEW.id_client_organization = CONCAT('ORG', LPAD(CAST(prev_id_number + 1 AS VARCHAR), 12, '0'));
	RETURN NEW;
 END;
 $BODY$
 LANGUAGE 'plpgsql';
